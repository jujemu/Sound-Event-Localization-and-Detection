...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        data_gen_test = cls_data_generator.DataGenerator(
            ...
            feat_label_dir=params['feat_label_dir'], shuffle=False, per_file=params['project_output'],
        ...
        if params['project_output']:
            # Dump results in project output format for calculating final scores
            project_dump_folder = os.path.join(params['project_dir'], '{}_{}_{}'.format(task_id, params['dataset'], params['mode']))
            cls_feature_class.create_folder(project_dump_folder)
            print('Dumping recording-wise results in: {}'.format(project_dump_folder))
            ...
            for file_cnt in range(test_sed_pred.shape[0]//frames_per_file):
                output_file = os.path.join(project_dump_folder, test_filelist[file_cnt].replace('.npy', '.csv'))
