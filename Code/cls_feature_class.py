...
class FeatureClass:
    ...
    def preprocess_features(self):
        ...
        if self._is_eval:
            ...
            arr = os.listdir(self._feat_dir)
            with tqdm(enumerate(arr), total=len(arr), desc='Preprocess ') as iterator:
                for file_cnt, file_name in iterator:
                    log = '{}: {}'.format(file_cnt, file_name)
                    iterator.set_postfix_str(log)
        ...
        arr = os.listdir(self._feat_dir)
        with tqdm(enumerate(arr), total=len(arr), desc='Preprocess Norm ') as iterator:
            for file_cnt, file_name in iterator:
                log = '{}: {}'.format(file_cnt, file_name)
                iterator.set_postfix_str(log)
                feat_file = np.load(os.path.join(self._feat_dir, file_name))
    ...
    def extract_all_labels(self):
        ...
        arr = os.listdir(self._desc_dir)
        with tqdm(enumerate(arr), total=len(arr), desc='Extract label ') as iterator:
            for file_cnt, file_name in iterator:
                log = '{}: {}'.format(file_cnt, file_name)
                iterator.set_postfix_str(log)
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
